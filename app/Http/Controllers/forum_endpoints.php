    ...
    switch ($action) {
        case 'create_forum':
            ...
            if ($visibility === 'class' && empty($class_id)) {
                sendResponse(400, null, 'Class selection is required for class-only forums');
            }
            ...
            if ($class_id) {
                ...
                // Allow teacher or enrolled students to create forums for the class
                $stmt = $db->prepare("
            ...
            try {
                $stmt = $db->prepare("
                    INSERT INTO forums (created_by, title, description, category, visibility, class_id, start_date, end_date)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ");
                ...
                $stmt = $db->prepare("INSERT INTO forum_members (forum_id, user_id, role) VALUES (?, ?, 'admin')");
                $stmt->execute([$forum_id, $user_id]);
                
                // Update member count
                $stmt = $db->prepare("UPDATE forums SET member_count = member_count + 1 WHERE id = ?");
        ...
        case 'create_post':
            ...
            if ($post_type === 'poll' && count($poll_options) < 2) {
                sendResponse(400, null, 'Poll must have at least 2 options');
            }
            
            $stmt = $db->prepare("SELECT id FROM forum_members WHERE forum_id = ? AND user_id = ?");
            $stmt->execute([$forum_id, $user_id]);
            if (!$stmt->fetch()) {
            ...
            try {
                $stmt = $db->prepare("
                    INSERT INTO forum_posts (forum_id, author_id, title, content, category, post_type)
                    VALUES (?, ?, ?, ?, ?, ?)
                ");
                ...
                foreach ($attachments as $attachment) {
                    $stmt = $db->prepare("
                        INSERT INTO post_attachments (post_id, file_url, file_name, file_type, file_size)
                        VALUES (?, ?, ?, ?, ?)
                    ");
                    $stmt->execute([
                        $post_id,
                ...
                if ($post_type === 'poll') {
                    foreach ($poll_options as $option_text) {
                        $stmt = $db->prepare("INSERT INTO poll_options (post_id, option_text) VALUES (?, ?)");
                        $stmt->execute([$post_id, $option_text]);
                    }
                }
                
                $stmt = $db->prepare("UPDATE forums SET post_count = post_count + 1 WHERE id = ?");
                $stmt->execute([$forum_id]);
        ...
        case 'edit_post':
            ...
            $stmt = $db->prepare("SELECT author_id FROM forum_posts WHERE id = ? AND is_deleted = FALSE");
        ...
        case 'delete_post':
            ...
            $stmt = $db->prepare("
                ...
                FROM forum_posts fp
                LEFT JOIN forum_members fm ON fp.forum_id = fm.forum_id AND fm.user_id = ?
            ...
            $stmt = $db->prepare("UPDATE forum_posts SET is_deleted = TRUE, deleted_at = NOW() WHERE id = ?");
            $stmt->execute([$post_id]);
        ...
        case 'get_forums':
            ...
            switch ($sort) {
                case 'popular':
                    $order_by = 'fs.post_count DESC, fs.member_count DESC';
                    break;
                case 'name':
                    $order_by = 'f.title ASC';
                    break;
                default:
                    $order_by = 'fs.last_activity DESC';
                    break;
            }
            ...
            $query = "
                ...
                INNER JOIN forum_members fm ON f.id = fm.forum_id AND fm.user_id = ?
        ...
        case 'get_posts':
            ...
            // Add filter for user's forums if no specific forum or post or post_ids
            if (!$post_id && !$forum_id && !$post_ids) {
                $where_clause .= " AND EXISTS (
                    SELECT 1 FROM forum_members fm WHERE fm.forum_id = fp.forum_id AND fm.user_id = ?
                )";
                $params[] = $user_id;
            }
            ...
            if ($forum_id && !$post_id) {
                $stmt = $db->prepare("UPDATE forum_posts SET view_count = view_count + 1 WHERE forum_id = ?");
                $stmt->execute([$forum_id]);
            }
        ...
        case 'create_comment':
            ...
            try {
                ...
                $stmt = $db->prepare("UPDATE forum_posts SET reply_count = reply_count + 1 WHERE id = ?");
                $stmt->execute([$post_id]);
                
                $stmt = $db->prepare("SELECT author_id, title FROM forum_posts WHERE id = ?");
                $stmt->execute([$post_id]);
                $post = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        case 'get_comments':
            ...
            switch ($sort) {
                case 'popular':
                case 'top':
                    // Sort by reaction_count + reply_count (top comments)
                    $order_by = '(c.reaction_count + COALESCE((SELECT COUNT(*) FROM comments WHERE parent_id = c.id AND is_deleted = FALSE), 0)) DESC, c.reaction_count DESC, c.created_at DESC';
                    break;
                case 'oldest':
            ...
            // Build tree structure (only top-level comments) using deep copy
            ...
            sendResponse(200, ['comments' => $tree]);
            break;
        ...
        case 'pin_post':
            ...
            $stmt = $db->prepare("
                ...
                FROM forum_posts fp
                JOIN forum_members fm ON fp.forum_id = fm.forum_id
            ...
            if ($pin) {
                $stmt = $db->prepare("SELECT COUNT(*) FROM forum_posts WHERE forum_id = ? AND is_pinned = TRUE");
            ...
            $stmt = $db->prepare("UPDATE forum_posts SET is_pinned = ? WHERE id = ?");
        ...
        case 'mute_user':
            ...
            $stmt = $db->prepare("
                SELECT role FROM forum_members 
                WHERE forum_id = ? AND user_id = ?
            ");
            $stmt->execute([$forum_id, $user_id]);
            ...
            if (!in_array($role, ['admin', 'moderator'])) {
                sendResponse(403, null, 'Not authorized to mute users');
            }
            
            $stmt = $db->prepare("
                INSERT INTO muted_users (forum_id, user_id, muted_by, reason, muted_until)
                VALUES (?, ?, ?, ?, ?)
        ...
        case 'get_forum_details':
            ...
            $stmt = $db->prepare("
                ...
                LEFT JOIN forum_statistics fs ON f.id = fs.forum_id
                LEFT JOIN users u ON f.created_by = u.id
                LEFT JOIN forum_members fm ON f.id = fm.forum_id AND fm.user_id = ?
                WHERE f.id = ?
            ");
        ...
        case 'join_forum':
            ...
            try {
                // Add user to forum
                $stmt = $db->prepare("INSERT INTO forum_members (forum_id, user_id, role) VALUES (?, ?, 'member')");
                $stmt->execute([$forum_id, $user_id]);
        ...
        case 'leave_forum':
            ...
            $stmt = $db->prepare("SELECT role FROM forum_members WHERE forum_id = ? AND user_id = ?");
            $stmt->execute([$forum_id, $user_id]);
            $role = $stmt->fetchColumn();
            ...
            try {
                ...
                $stmt = $db->prepare("DELETE FROM forum_members WHERE forum_id = ? AND user_id = ?");
                $stmt->execute([$forum_id, $user_id]);
        ...
        case 'mute_forum':
            ...
            $stmt = $db->prepare("INSERT INTO muted_users (forum_id, user_id, muted_by) VALUES (?, ?, ?)");
        ...
        case 'unmute_forum':
            ...
            $stmt = $db->prepare("DELETE FROM muted_users WHERE forum_id = ? AND user_id = ?");
            $stmt->execute([$forum_id, $user_id]);
        ...
        case 'favorite_forum':
            ...
            $stmt = $db->prepare("UPDATE forum_members SET is_favorite = TRUE WHERE forum_id = ? AND user_id = ?");
            $stmt->execute([$forum_id, $user_id]);
        ...
        case 'unfavorite_forum':
            ...
            $stmt = $db->prepare("UPDATE forum_members SET is_favorite = FALSE WHERE forum_id = ? AND user_id = ?");
            $stmt->execute([$forum_id, $user_id]);
            
            sendResponse(200, null, 'Forum removed from favorites');
            break;
        
        case 'get_forum_members':
            if ($method !== 'GET') {
                sendResponse(405, null, 'Method not allowed');
            }
        ...
        case 'promote_member':
            ...
            $stmt = $db->prepare("UPDATE forum_members SET role = 'admin' WHERE forum_id = ? AND user_id = ?");
        ...
        case 'update_forum':
            ...
            $stmt = $db->prepare("UPDATE forums SET title = ?, description = ? WHERE id = ?");
        ...
        case 'delete_forum':
            ...
            if ($role !== 'admin') {
                sendResponse(403, null, 'Only admins can delete forums');
            }
            
            $stmt = $db->prepare("DELETE FROM forums WHERE id = ?");
            $stmt->execute([$forum_id]);
        ...
        case 'get_comment_detail':
            ...
            $stmt = $db->prepare("
                ...
                FROM comments c
                JOIN users u ON c.author_id = u.id
                LEFT JOIN forum_posts fp ON c.post_id = fp.id
