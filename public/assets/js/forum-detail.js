...
function initEventListeners() {
    ...
    document.getElementById('btnCreatePostForForum').addEventListener('click', () => {
        const forumId = forumState.forumId || new URLSearchParams(window.location.search).get('id');
        window.location.href = `create-post.html?forum=${forumId}`;
    });
    
    document.getElementById('btnJoin').addEventListener('click', joinForum);
    ...
    document.addEventListener('click', function(e) {
        ...
        if (!menu.contains(e.target) && !btnMore.contains(e.target)) {
            menu.classList.remove('show');
        }
    });
    
    document.getElementById('muteOption').addEventListener('click', toggleMute);
    document.getElementById('favoriteOption').addEventListener('click', toggleFavorite);
    document.getElementById('manageOption').addEventListener('click', manageForum);
    document.getElementById('leaveOption').addEventListener('click', leaveForum);
...
function renderComments(comments) {
    ...
    container.innerHTML = sortedComments.map((comment, index) => renderCommentItemForumDetail(comment, 0, index === sortedComments.length - 1)).join('');
}
...
function renderCommentItemForumDetail(comment, depth = 0, isLastChild = false) {
    const totalScore = (comment.reaction_count || 0) + (comment.reply_count || 0);
    const hasHighScore = totalScore > 0 && depth === 0;
    const isCollapsed = collapsedCommentsForumDetail.has(comment.id);
        ...
        <div class="comment-thread" data-comment-id="${comment.id}" style="position: relative; padding-left: ${depth > 0 ? indentAmount + 32 : 0}px; padding-bottom: ${isLastInThread ? '0' : '8px'}; margin-bottom: ${depth === 0 ? '16px' : '0'};">
            ...
            ${depth > 0 ? `
                <div style="position: absolute; left: ${indentAmount + 7}px; top: 0px; z-index: 2;">
                    <button onclick="toggleCommentCollapseForumDetail(${comment.id})" style="width: ${buttonSize}px; height: ${buttonSize}px; border-radius: 50%; border: 1px solid #d1d5db; background: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 0; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                        ${isCollapsed ? `
            ...
            ` : `
                ...
                ${hasReplies && !isCollapsed ? `
                    <div style="margin-top: 12px;">
                        ${comment.replies.map((reply, index) => renderCommentItemForumDetail(reply, depth + 1, index === comment.replies.length - 1)).join('')}
                    </div>
                ` : ''}
