        ...
        <div class="comments-container">
            ...
            <div id="repliesContainer" style="position: relative;">
                ${comment.replies && comment.replies.length > 0 
                    ? comment.replies.map((reply, index) => renderReplyItem(reply, 0, comment.id, index === comment.replies.length - 1)).join('')
...
function renderReplyItem(reply, depth = 0, parentId = null, isLastChild = false) {
    const isCollapsed = collapsedComments.has(reply.id);
    const hasReplies = reply.replies && reply.replies.length > 0;
    const indentAmount = depth * 40;
    const buttonSize = 18;
    const buttonCenter = indentAmount + 16; // Center of button
    const isLastInThread = isLastChild && !hasReplies;
    
    return `
        <div class="comment-thread" data-comment-id="${reply.id}" style="position: relative; padding-left: ${indentAmount + 32}px; padding-bottom: ${isLastInThread ? '0' : '8px'};">
            <!-- Vertical line from parent's button to this comment's button -->
            ${depth > 0 ? `
                <div style="position: absolute; left: ${indentAmount - 40 + 16}px; top: -8px; bottom: ${isLastInThread ? 'auto' : '100%'}; width: 2px; background: #d1d5db; z-index: 0; height: ${isLastInThread ? buttonSize / 2 + 8 + 'px' : '100%'};"></div>
            ` : ''}
            
            <!-- Collapse button container -->
            <div style="position: absolute; left: ${indentAmount + 7}px; top: 0px; z-index: 2;">
                <button onclick="toggleCommentCollapse(${reply.id})" style="width: ${buttonSize}px; height: ${buttonSize}px; border-radius: 50%; border: 1px solid #d1d5db; background: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 0; box-shadow: 0 1px 2px rgba(0,0,0,0.05); position: relative;">
                    ${isCollapsed ? `
                ...
                <!-- Vertical line from button down to children (only if not collapsed and has replies) -->
            ...
            ${isCollapsed ? `
                <div style="padding: 4px 0; margin-left: 12px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <div class="comment-author-info" style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <span style="font-size: 12px; color: #9ca3af;">• ${reply.reply_count || 0} ${reply.reply_count === 1 ? 'reply' : 'replies'}</span>
                        </div>
                    </div>
                </div>
            ` : `
                ...
                ${hasReplies ? `
                    <div style="margin-top: 4px;">
                        ${reply.replies.map((nestedReply, index) => renderReplyItem(nestedReply, depth + 1, reply.id, index === reply.replies.length - 1)).join('')}
                    </div>
                ` : ''}
            `}
        </div>
